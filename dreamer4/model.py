...
class TokenLayout:
    ...
    def modality_ids(self) -> torch.Tensor:
        ...
        if self.n_latents > 0:
            parts.append(
                torch.full((self.n_latents,), int(Modality.LATENT), dtype=torch.int32)
            )
        for m, n in self.segments:
            if n > 0:
                parts.append(torch.full((n,), int(m), dtype=torch.int32))
        return (
            torch.cat(parts, dim=0) if parts else torch.zeros((0,), dtype=torch.int32)
        )
...
def temporal_patchify(videos_btchw: torch.Tensor, patch: int) -> torch.Tensor:
    ...
    x = rearrange(videos_btchw, "b t c (h p1) (w p2) -> b t (h w) (c p1 p2)", p1=patch, p2=patch)
    return x


def temporal_unpatchify(
    patches_btnd: torch.Tensor, H: int, W: int, C: int, patch: int
) -> torch.Tensor:
    """
...
class MAEReplacer(nn.Module):
    ...
    def forward(
        self, patches_btnd: torch.Tensor, disable_mae=False
...
class SpaceSelfAttentionModality(nn.Module):
    ...
    def __init__(self, modality_ids: torch.Tensor, mode: str):
        ...
        self.attn = GroupedQueryAttention(
            self.d_model, **self._config.select("attention")
        )
    ...
    def __init__(self, layer_index: int, modality_ids: torch.Tensor, space_mode: str):
        ...
        self.space_attn = nn.Sequential(
            SpaceSelfAttentionModality(
                modality_ids, space_mode, **self._config.select()
            ),
...
class BlockCausalTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.gradient_checkpointing and self.training:
            x = checkpoint_sequential(self.layers, self.depth, x, use_reentrant=False)
...
class Encoder(nn.Module):
    ...
    def __init__(self, n_patches: int, d_patch: int):
        ...
        self.layout = TokenLayout(
            n_latents=self.n_latents, segments=((Modality.IMAGE, self.n_patches),)
        )
        ...
        self.mae = MAEReplacer(
            d_model=self.d_model, p_min=self.mae_p_min, p_max=self.mae_p_max
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, n_patches: int, d_patch: int):
        ...
        self.transformer = BlockCausalTransformer(
            modality_ids=modality_ids, space_mode="decoder", **self._config.select()
        )
...
class Tokenizer(nn.Module):
    ...
    def __init__(self, C, H, W):
        ...
        self.encoder = Encoder(
            n_patches=self.n_patches, d_patch=self.d_patch, **self._config.select()
        )
...
def pack_bottleneck_to_spatial(
    z_btLd: torch.Tensor, *, n_spatial: int, k: int
) -> torch.Tensor:
    """
...
def _sample_step_excluding_dmin(
    ...
    step_idx = torch.randint(
        low=0, high=max(1, emax), size=(B, T), device=device, dtype=torch.long
    )
...
class Dynamics(nn.Module):
    ...
    @classmethod
    def from_checkpoint(
        cls, checkpoint: Path, tokenizer: Tokenizer, device: str = "cpu"
    ) -> tuple["Dynamics", dict]:
    ...
    def save_checkpoint(
        ...
        if full_config is not None:
            ...
            config = (
                self._conf.asdict() if hasattr(self._conf, "asdict") else self._conf
            )
        else:
            config = {}
    ...
    def train_step(
        ...
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            ...
            sigma_full, sigma_idx_full = _sample_tau_for_step(
                device, B, T, self.k_max, step_idx_full
            )
            ...
            flow_per = (
                (z1_hat_emp.float() - z1[:B_emp].float()).pow(2).mean(dim=(2, 3))
            )  # (B_emp,T)
            ...
            if do_boot:
                ...
                sigma_idx_plus = sigma_idx_self + (
                    torch.tensor(self.k_max, device=device, dtype=torch.float32)
                    * d_half
                ).to(torch.long)

                z1_hat_half1, _ = self(
                    actions[B_emp:] if actions is not None else None,
                    step_idx_half,
                    sigma_idx_self,
                    z_tilde_self,
                ...
                boot_per = (1.0 - sigma_self).pow(2) * (vhat_sigma - vbar_target).pow(
                    2
                ).mean(dim=(2, 3))  # (B_self,T)
